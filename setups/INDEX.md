...
| [`impeccable-casenote/`](impeccable-casenote/design.md) | designed | Claude Code | local | Vendored, version-pinned Impeccable detector plus a Casenote denylist checker, sharing one exit-code contract. `brands/alan-personal.md` stays the single source of truth; no upstream fork. Design only so far. |
